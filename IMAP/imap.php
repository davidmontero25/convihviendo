...
class ImapReader {
    ...
    public function connect() {
        if ($this->canConnect()) {
            $this->box = @imap_open("{{$this->host}:{$this->port}/imap/ssl/novalidate-cert}INBOX", $this->user, $this->pass);
            if ($this->box !== false) {
                $this->_connected();
            } else {
...
//        }
    ...
    private function _fetchHeader($mbox, $id) {
        ...
        $mail->from = $this->_fix(isset($header->from) ? ($header->from) : (''));
        $mail->to = $this->_fix(isset($header->to) ? ($header->to) : (''));
